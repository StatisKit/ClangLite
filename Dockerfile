...
RUN [ $BUILD = "true" ] && for recipe in PyClangLite/conda/*/; do echo "anaconda upload \`conda build" $recipe "--output\` --user statiskit --force" >> $HOME/post-link.sh; done; || [ $BUILD = "false" ]
